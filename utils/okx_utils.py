...
if IS_DEVELOPMENT:
    ...
    print(f"[okx_utils] 检查.env_dev路径: {env_path}")
    if os.path.exists(env_path):
        print(f"[okx_utils] 检测到.env_dev文件，开始加载环境变量...")
        ...
        print(f"[okx_utils] 未找到.env_dev文件: {env_path}")
...
def send_bark_notification(title, content, group=None):
    ...
    bark_group = group or os.getenv("BARK_GROUP", "OKX自动交易")
    if not bark_key:
